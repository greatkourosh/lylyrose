...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function triggerConflictingCurlOptionDeprecations(array $options) : void
    {
        ...
        foreach ($options['curl'] as $option => $_) {
            ...
            if ($replacement !== null) {
                \trigger_deprecation('guzzlehttp/guzzle', '7.11', \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed request handling. Use %s instead.', $name, $replacement));
                continue;
            }
            \trigger_deprecation('guzzlehttp/guzzle', '7.11', \sprintf('Passing %s in the "curl" request option is deprecated; guzzlehttp/guzzle 8.0 will reject this option because it conflicts with Guzzle-managed cURL internals.', $name));
        }
    }
