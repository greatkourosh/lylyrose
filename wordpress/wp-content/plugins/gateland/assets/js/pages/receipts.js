...
document.addEventListener('alpine:init', () => {
    ...
    Alpine.data("card2cardReceipts", ()=>({
        ...
        filters: {
            statuses: [
                {key: 'pending', value: 'نیازمند بررسی'},
                {key: 'accepted', value: 'تایید شده'},
        ...
        async getPageData(){
            ...
            const newTableFilters = this.tableFilters;
            if(newTableFilters.card_number){
                newTableFilters.card_number = gatelandCardNumberToString(newTableFilters.card_number)
            }
            const filtersObj = gatelandGenerateFiltersObject(newTableFilters);
            gatelandSetUrlQueryParams('gateland-receipts', filtersObj);

            try{

                const result = await gatelandApiRequest('gateland/card-to-card/receipt/index', {
                    method: 'POST',
        ...
        async getFilters(){
            ...
            try{
                ...
                const result = await gatelandApiRequest('gateland/card-to-card/receipt/filters', {
                    method: 'POST'
                })
        ...
        download(nonce){
            ...
            window.location = gatelandBaseUrl + 'gateland/card-to-card/receipt/index?' + gatelandBuildQueryString(filtersObj);
        },
