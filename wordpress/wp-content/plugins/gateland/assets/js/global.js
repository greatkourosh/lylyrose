...
window.addEventListener('load', function (){
    try {
        //dynamic array
        const otherSubMenu = {
            'gateland-transaction' : 'gateland-transactions',
            'gateland-gateway' : 'gateland-gateways',
            'gateland-gateways-add' : 'gateland-gateways',
            'gateland-gateways-edit' : 'gateland-gateways',
            'gateland-receipt' : 'gateland-receipts'
        }

        //static for all wp plugin
        const pageName = gatelandGetQueryParam('page');
        if(!pageName){
            return
        }
        const adminMenuEl = document.getElementById('adminmenu');

        let href = `admin.php?page=${pageName}`;
        let currentSubmenu = adminMenuEl.querySelector(`.wp-submenu a[href="${href}"]`);
        if(!currentSubmenu){
            href = `admin.php?page=${otherSubMenu[pageName]}`;
            currentSubmenu =  adminMenuEl.querySelector(`.wp-submenu a[href="${href}"]`)
            const wpHasSubmenuEl = currentSubmenu.closest('.wp-has-submenu')
            currentSubmenu.closest('li').classList.add('current');
            wpHasSubmenuEl.classList.add('wp-menu-open', 'wp-has-current-submenu');
            wpHasSubmenuEl.classList.remove('wp-not-current-submenu');
            wpHasSubmenuEl.querySelector('.wp-has-submenu').classList.add('wp-menu-open', 'wp-has-current-submenu');
            wpHasSubmenuEl.querySelector('.wp-has-submenu').classList.remove('wp-not-current-submenu');
        }
    }catch (error){
...
function gatelandCopyToClipboard(el, label = '') {
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(textValue)
            .then(() => {
                gatelandNotyf.success((label ? label + ' ' : '') + 'با موفقیت کپی شد');
                if (textCopyEl) {
            ...
            .catch((err) => {
                ...
                this.gatelandFallbackCopyToClipboard(textValue, textCopyEl);
            });
    } else {
        this.gatelandFallbackCopyToClipboard(textCopyEl, textValue, label);
    }
}

function gatelandFallbackCopyToClipboard(el, text, label){
...
async function gatelandApiRequest(url, options = {}) {
    ...
    if(headers.nonce !== null){
        headers['X-WP-Nonce'] = gateland.nonce
    }
...
function gatelandConvertPriceToWords(amount) {
    if (isNaN(amount)) return 'عدد معتبر وارد کنید';

    if(amount > 100000000000){
        return 'مبلغ از حد مجاز بالاتر است';
    }
    ...
    const groupNames = ['', 'هزار', 'میلیون', 'میلیارد', 'بیلیون', 'تریلیون'];
    ...
    for (let i = numStr.length; i > 0; i -= 3) {
        ...
        groups.push(parseInt(numStr.slice(start, i), 10));
    }
    groups.reverse();
    ...
    if (amount < 0) result = 'منفی ' + result;
    return result;
}

function gatelandFormatCardNumber(cardNumber) {
    if(!cardNumber){
        return null;
    }
    ...
    for (let i = 0; i < cardNumber.length; i++) {
        formated += cardNumber.charAt(i);
        if((i+1 < cardNumber.length) && (i+1) % 4 === 0){
            formated += '-';
        }
    }
    return formated;
