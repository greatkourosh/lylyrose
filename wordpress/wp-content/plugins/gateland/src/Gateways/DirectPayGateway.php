...
class DirectPayGateway extends BaseGateway implements \Nabik\Gateland\Gateways\Features\FreeFeature, InquiryFeature, ShaparakFeature {

	protected string $name = 'دایرکت‌پی';

	protected string $description = 'directpay.click';

	protected string $url = 'https://l.nabik.net/directpay';
	...
	public function request( Transaction $transaction ): void {
		...
		try {
			...
			$url      = sprintf( 'https://api.directpay.%s/api/pardakht/create', $this->get_tld() );
	...
	public function inquiry( Transaction $transaction ): bool {
		...
		try {
			...
			$url      = sprintf( 'https://api.directpay.%s/api/pardakht/verify', $this->get_tld() );
			$response = $this->curl( $url, json_encode( $parameters ), $headers, [ CURLOPT_TIMEOUT => 10 ] );
	...
	public function redirect( Transaction $transaction ) {
		...
		return wp_redirect(
			sprintf( 'https://api.directpay.%s/api/pardakht/payment/?token=%s', $this->get_tld(),
	...
	public function options(): array {
		return [
			[
				...
				'key'         => 'gateway_id',
				'type'        => 'text',
				'description' => 'شناسه درگاه را از آدرس https://my.directpay.finance دریافت کنید.',
			],
			[
				'label' => 'کلید رمزنگاری',
				'key'   => 'encryption_key',
	...
	private function get_tld(): string {
		return ( $this->options['non-iran-host'] ?? false ) ? 'click' : 'finance';
	}
}