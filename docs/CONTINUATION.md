...
- **Local DB is still branded "Aromaland"** (spotted 2026-09-26). `blogname`, and the `wp_mail_smtp` `from_name`/`from_email` (`admin@aromalnd.test`), are inherited from the Aroma Store source project and were never re-branded in the local volume. Local-only — the production site at `lylyrose.ir` is correctly branded. Cosmetic for tests, but it must be fixed before any **re-deploy**, since a fresh dump would ship the wrong store name.
